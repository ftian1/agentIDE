...
/// Build-time constants generated by build.rs
mod build_info {
    include!(concat!(env!("OUT_DIR"), "/build_info.rs"));
}

/// Embedded dist tarball (generated by build.rs from the entire dist/ directory).
/// Contains all OTA-managed files: frontend.tar.gz, agent binaries, pricing.json,
/// manifest.json. Extracted to cache on every startup as the baseline.
const EMBEDDED_DIST_TARBALL: &[u8] =
    include_bytes!(concat!(env!("OUT_DIR"), "/embedded_dist.tar.gz"));
...
pub fn run() {
    ...
    log_msg!(&log_path_str, "[remote-ai-ide] Build: {} (git {})", build_info::BUILD_TIMESTAMP, build_info::GIT_HASH);
    ...
    builder
        .setup(move |app| {
            ...
            // ── Extract embedded dist to cache ──
            // Always extract on startup so the cache baseline matches the
            // binary's embedded version. This includes ALL OTA-managed files:
            // frontend.tar.gz, agent binaries, pricing.json, manifest.json.
            // The OTA updater will download newer versions if available.
            let cache_dir_clone = cache_for_window.clone();
            if !EMBEDDED_DIST_TARBALL.is_empty() {
                let _ = std::fs::create_dir_all(&cache_dir_clone);
                log_msg!(&log_path_clone, "[remote-ai-ide] Extracting embedded dist to cache...");
                match extract_tar_gz_bytes(EMBEDDED_DIST_TARBALL, &cache_dir_clone) {
                    Ok(()) => log_msg!(&log_path_clone, "[remote-ai-ide] Embedded dist extracted to {}", cache_dir_clone.display()),
                    Err(e) => log_msg!(&log_path_clone, "[remote-ai-ide] Failed to extract embedded dist: {e}"),
                }
            }
            // After extraction, also unpack frontend.tar.gz into frontend/ subdir
            // if the cache doesn't already have it (first run after fresh extract).
            let frontend_tar = cache_dir_clone.join("frontend.tar.gz");
            let cache_frontend = cache_dir_clone.join("frontend");
            if frontend_tar.exists() {
                let _ = std::fs::create_dir_all(&cache_frontend);
                log_msg!(&log_path_clone, "[remote-ai-ide] Unpacking frontend.tar.gz to {}...", cache_frontend.display());
                if let Err(e) = extract_tar_gz_path(&frontend_tar, &cache_frontend) {
                    log_msg!(&log_path_clone, "[remote-ai-ide] Failed to unpack frontend: {e}");
                }
            }
...
/// Extract a tar.gz byte slice to a directory.
fn extract_tar_gz_bytes(data: &[u8], dest: &std::path::Path) -> Result<(), String> {
    let decoder = flate2::read::GzDecoder::new(data);
    let mut archive = tar::Archive::new(decoder);
    archive.unpack(dest).map_err(|e| format!("extract tar.gz: {e}"))?;
    Ok(())
}

/// Extract a tar.gz file to a directory.
fn extract_tar_gz_path(src: &std::path::Path, dest: &std::path::Path) -> Result<(), String> {
    let data = std::fs::read(src).map_err(|e| format!("read tar.gz: {e}"))?;
    extract_tar_gz_bytes(&data, dest)
}
