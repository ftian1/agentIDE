...
### 4.32 构建路径与中间产物（单一事实来源，禁止偏离）

本节是构建系统的**唯一权威定义**。任何对构建流程的改动必须同步更新本节。
...
ide/                              # 仓库根
├── apps/frontend/                # 前端项目根
│   ├── index.html                # ★ 源码 HTML（splash screen 等在此编辑）
│   ├── src/                      # React/TS 源码
│   │   └── main.tsx              # ★ 入口 TS
│   ├── dist/                     # ★ Tauri 的 frontendDist（Vite 输出到这里）
│   │   ├── index.html            #    构建产物 — Tauri 从此处嵌入到 loader.exe
│   │   └── assets/               #    JS/CSS/images
│   └── src-tauri/                # Tauri Rust 项目根
│       ├── Cargo.toml            #    package = "remote-ai-ide"
│       ├── tauri.conf.json       #    frontendDist: "../dist" = apps/frontend/dist/
│       ├── build.rs              #    编译时嵌入 manifest → OUT_DIR
│       └── src/
│           ├── lib.rs            #    run() 入口 + 窗口创建 + 启动版本比较
│           ├── main.rs           #    调 lib::run()
│           ├── manifest.rs       #    Manifest 类型 + EMBEDDED_MANIFEST_JSON
│           ├── updater.rs        #    OTA 后台更新逻辑
│           └── ...
├── dist/                         # ★ OTA 发布目录（git tracked，--force-add）
│   ├── loader.exe                #    = remote-ai-ide.exe（Tauri 构建产物，重命名）
│   ├── frontend.tar.gz           #    = apps/frontend/dist/ 的 tar 包（Vite 输出）
│   ├── agent-linux-x86_64        #    = remote-agent-host（Linux 构建产物）
│   ├── agent-windows-x86_64      #    = agent.exe（Windows 交叉编译产物）
│   ├── pricing.json              #    定价配置文件
│   └── manifest.json             #    OTA manifest（version + 各文件 SHA256）
├── scripts/release.sh            # ★ 唯一构建入口，禁止绕过
├── crates/
│   ├── shared-protocol/          # 前后端共享的 MessagePack 协议定义
│   └── remote-agent-host/        # 上传到远程 Linux 的 agent 二进制
└── design.md                     # 本文件
```
...
Step             输出                              去向
──────────────────────────────────────────────────────────────────
[1/4] Frontend   apps/frontend/dist/*              ──→ tar → dist/frontend.tar.gz
                                                        ──→ Tauri 嵌入到 loader.exe（同一份文件）
[2/4] Agent      dist/agent-linux-x86_64            git tracked
                 dist/agent-windows-x86_64          git tracked
[3/4] Manifest   dist/manifest.json                ★ 必须在 Tauri build 之前
                 dist/pricing.json                 git tracked
[4/4] Tauri      dist/loader.exe                   git tracked
```

**关键约束（曾犯过错的）：**

1. **Vite 输出路径 = Tauri frontendDist**：`release.sh` 的 `--outDir` 必须是 `dist`（相对于 `apps/frontend/`），即 `apps/frontend/dist/`。`tauri.conf.json` 的 `frontendDist` 是 `"../dist"`（相对于 `apps/frontend/src-tauri/`），解析后同样是 `apps/frontend/dist/`。**两者必须指向同一目录**，否则 `frontend.tar.gz`（OTA 下载）和 `loader.exe`（内嵌）的 HTML 不同。
   - ❌ 旧代码：`--outDir ../../dist-www`（repo 根下的 dist-www/） — 与 Tauri 不同步
   - ✅ 新代码：`--outDir dist`（apps/frontend/dist/） — 与 Tauri 同步

2. **Manifest 在 Tauri 之前生成**：`build.rs` 在 Tauri 编译时读取 `dist/manifest.json` 嵌入到 `loader.exe`。所以 `[3/4] Manifest` 必须在 `[4/4] Tauri` 之前运行。完整 build 时 manifest 包含所有文件 SHA256；`--tauri-only` 时 manifest 仅含 version（`"files": {}`）。

3. **dist/ 是 gitignored 但 git tracked**：`git add -f dist/` 强制添加。每次 release.sh 后必须同时 commit 源码 + dist/，否则 OTA 客户端看不到更新。

4. **loader.exe 不在 manifest 中**：manifest 只列 OTA 可更新的文件，loader.exe 本身通过手动下载更新。

#### Tauri 构建时的文件嵌入机制

```
cargo xwin build -p remote-ai-ide
  → build.rs 执行:
      1. 复制 dist/manifest.json → OUT_DIR/embedded_manifest.json
         (若 dist/manifest.json 不存在则写 "0.0.0.dev" 占位)
      2. tauri_build::build() 调用:
         - 读 tauri.conf.json → frontendDist = "../dist"
         - 遍历 apps/frontend/dist/ 下所有文件
         - zstd 压缩每个文件 → 嵌入到 loader.exe 的资源段
  → 编译 Rust 源码:
      - manifest.rs: include_str!(OUT_DIR/embedded_manifest.json)
      - uploader.rs: include_bytes!(binaries/remote-agent-host-*)
  → 链接: x86_64-pc-windows-msvc → dist/loader.exe
```

#### Windows 运行时缓存路径

```
%LOCALAPPDATA%/remote-ai-ide/cache/    # = cache_dir()
├── manifest.json                      # OTA 下载的 manifest（或 embedded 写入的 baseline）
├── frontend.tar.gz                    # OTA 下载
├── frontend/                          # tar.gz 解压产物
│   └── index.html
├── agent-windows-x86_64               # OTA 下载
├── agent-linux-x86_64                 # OTA 下载
└── pricing.json                       # OTA 下载
```
...
```
<UTC日期>.<git-short-hash>
例: 2026-07-01.3e52785

生成: date -u +%Y-%m-%d + git rev-parse --short=7 HEAD
